        ...
        where T: FromStr
    {
        ...
        for (i,a) in self.argv.iter().enumerate() {
            ...
            let arg_str = match arg.matches(a) {
                Ok(ArgMatch2::NoMatch) => continue,
                Ok(ArgMatch2::AtOffset(i)) => &a[i..],
                Ok(ArgMatch2::NextArg) => match self.argv.get(i+1) {
                    Some(next) => next,
                    None => { self.errs.push(ArgError::MissingValue); continue }
                },
                Err(e) => { self.errs.push(e); continue },
            };
            match arg_str.parse() {
                Ok(arg_val) => {
                    *result = arg_val;
